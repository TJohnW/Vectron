...
Aamap.prototype = {
    ...
    deleteObjs:function(selectedObjs) {
        for(var i = 0, ii = selectedObjs.length; i < ii; i++) {
            var index = this.aamapObjects.indexOf(selectedObjs[i]);
            if(index > -1) {
                this.vectron.gui.writeLog("Match!");
                this.aamapObjects.splice(index, 1);
                if(this.aamapObjects[index].obj != null)
                    this.aamapObjects[index].obj.remove();
            }
        }
    },
